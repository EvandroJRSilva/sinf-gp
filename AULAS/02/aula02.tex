...
\setbeamertemplate{background canvas}{%
	\tikz[remember picture,overlay] \node[opacity=0.15,inner sep=0pt] at (current page.center){\includegraphics[width=\paperwidth,height=\paperheight]{imagens/Grace_Episcopal_Church,_Galveston,_Texas.jpg}};
}
...
\setbeamertemplate{background canvas}{%
	\tikz[remember picture,overlay] \node[opacity=0.15,inner sep=0pt] at (current page.center){\includegraphics[width=\paperwidth,height=\paperheight]{imagens/Google_Headquarters_Google_Logo_(52639793897).jpg}};
}
...
\setbeamertemplate{background canvas}{%
	\tikz[remember picture,overlay] \node[opacity=0.15,inner sep=0pt] at (current page.center){\includegraphics[width=\paperwidth,height=\paperheight]{imagens/Australia_Victoria_relief_location_map.jpg}};
}
...
\setbeamertemplate{background canvas}{%
	\tikz[remember picture,overlay] \node[opacity=0.15,inner sep=0pt] at (current page.center){\includegraphics[width=\paperwidth,height=\paperheight]{imagens/Aerial_view_of_Apple_Park_dllu_(cropped).jpg}};
}
...
\setbeamertemplate{background canvas}{%
	\tikz[remember picture,overlay] \node[opacity=0.15,inner sep=0pt] at (current page.center){\includegraphics[width=\paperwidth,height=\paperheight]{imagens/Hospital.png}};
}
...
\setbeamertemplate{background canvas}{%
	\tikz[remember picture,overlay] \node[opacity=0.15,inner sep=0pt] at (current page.center){\includegraphics[width=\paperwidth]{imagens/The_new_logo_of_Johnson_&_Johnson.png}};
}
...
% SUBSECTION 3.1 ---------------------------------------------------------------
%-------------------------------------------------------------------------------
\subsection{Problema 1 - Startup EdTech Prestes a Perder Investimento}

\begin{frame}{Problema 1 - Startup EdTech Prestes a Perder Investimento}
	\begin{itemize}
		\justifying
		\item Startup desenvolve plataforma adaptativa para ensino técnico. Os investidores exigem:
			\begin{itemize}
				\justifying
...
\begin{frame}{Problema 1 - Startup EdTech Prestes a Perder Investimento}
	\begin{itemize}
		\justifying
		\item \textbf{Dilema}: comando rígido versus liderança inspiradora e empática.
		\item \textbf{Perguntas norteadoras}
		\begin{itemize}
			\item \textbf{Compreensão}
				\begin{itemize}
					\justifying
					\item Quais sinais indicam que a equipe está enfrentando problemas de motivação e desgaste?
					\item Qual é o papel do gerente do projeto em situações de alta pressão?
					\item Qual a diferença entre autoridade formal e liderança efetiva?
				\end{itemize}
			\item \textbf{Análise}
				\begin{itemize}
					\justifying
					\item Como o estilo de liderança pode influenciar o desempenho da equipe?
					\item De que forma a pressão por resultados pode afetar o clima organizacional?
					\item Quais consequências podem surgir se o gerente optar por aumentar o controle e a cobrança?
				\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \textbf{Tomada de decisão}
				\begin{itemize}
					\justifying
					\item Que atitudes o gerente pode tomar para recuperar a motivação da equipe?
					\item Como equilibrar metas agressivas com bem-estar da equipe?
					\item Que características definem um líder eficaz em projetos de alta incerteza?
				\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
...
% SUBSECTION 3.2 ---------------------------------------------------------------
%-------------------------------------------------------------------------------
\subsection{Problema 2 - Sistema Inteligente de Transporte Urbano}

\begin{frame}{Problema 2 - Sistema Inteligente de Transporte Urbano}
	\begin{itemize}
		\justifying
...
\begin{frame}{Problema 2 - Sistema Inteligente de Transporte Urbano}
	\begin{itemize}
		\item \textbf{Dilema}: gerenciar tarefas isoladas ou enxergar o projeto como sistema dinâmico interconectado.
		\item \textbf{Perguntas norteadoras}
		\begin{itemize}
			\item \textbf{Compreensão}
				\begin{itemize}
					\justifying
					\item Quais são os principais componentes do sistema de transporte urbano proposto?
					\item Que fatores externos influenciam diretamente o projeto?
					\item Por que projetos complexos devem ser tratados como sistemas interconectados?
				\end{itemize}
			\item \textbf{Análise}
				\begin{itemize}
					\justifying
					\item Como mudanças políticas podem afetar projetos de tecnologia pública?
					\item De que forma alterações legais impactam decisões técnicas no projeto?
					\item Como diferentes eventos externos podem gerar efeitos em cadeia no projeto?
				\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \textbf{Tomada de decisão}
				\begin{itemize}
					\justifying
					\item Que estratégias podem ajudar o gerente do projeto a lidar com mudanças no ambiente externo?
					\item Como monitorar continuamente fatores internos e externos ao projeto?
					\item Que ferramentas ou práticas ajudam a visualizar as interdependências de um sistema complexo?
				\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
...
% SUBSECTION 3.3 ---------------------------------------------------------------
%-------------------------------------------------------------------------------
\subsection{Problema 3 - Implantação de ERP em Indústria de Médio Porte}

\begin{frame}{Problema 3 - Implantação de ERP em Indústria de Médio Porte}
	\begin{itemize}
		\justifying
		\item Indústria metalúrgica com 320 funcionários decide substituir sistema legado por ERP moderno. Objetivos estratégicos declarados:
			\begin{itemize}
		...
		\item Durante as reuniões iniciais:
			...
			\end{itemize}
		\item Projeto é concluído no prazo e orçamento. Após 6 meses:
			\begin{itemize}
				\justifying
...
\begin{frame}{Problema 3 - Implantação de ERP em Indústria de Médio Porte}
	\begin{itemize}
		\justifying
		\item \textbf{Dilema}: entregar o que foi pedido versus entregar o que gera benefício real.
		\item \textbf{Perguntas norteadoras}
		\begin{itemize}
			\item \textbf{Compreensão}
				\begin{itemize}
					\justifying
					\item Quais eram os objetivos estratégicos da empresa ao implantar o ERP?
					\item Qual a diferença entre entregar funcionalidades e gerar valor para o negócio?
					\item Por que os usuários continuam utilizando planilhas paralelas após a implantação?
				\end{itemize}
			\item \textbf{Análise}
				\begin{itemize}
					\justifying
					\item Como o foco excessivo em requisitos solicitados pode prejudicar o resultado do projeto?
					\item De que forma a falta de alinhamento com os objetivos estratégicos afeta o retorno do investimento?
					\item Quais indicadores poderiam demonstrar se o projeto realmente gerou valor?
				\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Problema 3 - Implantação de ERP em Indústria de Médio Porte}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\justifying
				\item Que mudanças poderiam ser feitas durante o projeto para garantir geração de valor?
				\item Como priorizar funcionalidades em projetos complexos com múltiplos interesses?
				\item Como envolver usuários finais no processo de definição do que realmente gera valor?
			\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
...
% SUBSECTION 3.4 ---------------------------------------------------------------
%-------------------------------------------------------------------------------
\subsection{Problema 4 - Construção de Complexo Universitário em Área Urbana}

\begin{frame}{Problema 4 - Construção de Complexo Universitário em Área Urbana}
	\begin{itemize}
		\justifying
		\item Uma universidade privada adquire um terreno próximo a área residencial para expandir cursos de engenharia. O projeto inclui:
			\begin{itemize}
				\item 4 novos blocos.
				\item Laboratórios.
		...
		\item \textit{Stakeholders} identificados inicialmente:
			\begin{itemize}
				...
				\item Corpo docente.
			\end{itemize}
		\item Moradores da região não foram formalmente mapeados. Após início das obras:
			\begin{itemize}
				\justifying
...
\begin{frame}{Problema 4 - Construção de Complexo Universitário em Área Urbana}
	\begin{itemize}
		\justifying
		\item \textbf{Dilema}: velocidade e retorno sobre investimento versus engajamento social.
		\item \textbf{Perguntas norteadoras}
		\begin{itemize}
			\item \textbf{Compreensão}
				\begin{itemize}
					\justifying
					\item Quem são os principais stakeholders envolvidos nesse projeto?
					\item Quais grupos foram inicialmente considerados e quais foram negligenciados?
					\item Por que moradores da região podem ser considerados \textit{stakeholders} importantes?
				\end{itemize}
			\item \textbf{Análise}
				\begin{itemize}
					\justifying
					\item De que maneira a falta de envolvimento da comunidade afetou o andamento do projeto?
					\item Que impactos sociais e políticos podem surgir quando \textit{stakeholders} relevantes são ignorados?
					\item Como a resistência da comunidade pode gerar riscos financeiros e jurídicos?
				\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Problema 4 - Construção de Complexo Universitário em Área Urbana}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\justifying
				\item Que estratégias poderiam ter sido adotadas para envolver os moradores antes do início das obras?
				\item Como o gerente do projeto pode reconstruir o relacionamento com a comunidade após o conflito?
				\item Que critérios podem ser usados para priorizar o nível de engajamento de diferentes \textit{stakeholders}?
			\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
...
% SUBSECTION 3.5 ---------------------------------------------------------------
%-------------------------------------------------------------------------------
\subsection{Problema 5 - Desenvolvimento de Plataforma de Telemedicina para Região Norte}

\begin{frame}{Problema 5 - Desenvolvimento de Plataforma de Telemedicina para Região Norte}
	\begin{itemize}
		\justifying
		\item Uma \textit{healthtech} foi contratada pelo governo estadual para implantar telemedicina em municípios isolados. Equipe do projeto:
			\begin{itemize}
...
\begin{frame}{Problema 5 - Desenvolvimento de Plataforma de Telemedicina para Região Norte}
	\begin{itemize}
		\justifying
		\item \textbf{Dilema}: especialização técnica vs. cooperação interdisciplinar.
		\item \textbf{Perguntas norteadoras}
		\begin{itemize}
			\item \textbf{Compreensão}
				\begin{itemize}
					\justifying
					\item Quais são as diferentes áreas de conhecimento envolvidas no projeto de telemedicina?
					\item Quais sinais indicam que a equipe não está colaborando de forma eficaz?
					\item Por que projetos multidisciplinares exigem maior integração entre os participantes?
				\end{itemize}
			\item \textbf{Análise}
				\begin{itemize}
					\justifying
					\item Como a falta de comunicação entre especialistas pode afetar a qualidade do sistema final?
					\item De que forma a ausência de colaboração pode aumentar custos e atrasar o projeto?
					\item Quais conflitos surgem quando diferentes profissionais têm prioridades distintas?
				\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Problema 5 - Desenvolvimento de Plataforma de Telemedicina para Região Norte}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\justifying
				\item Que práticas o gerente do projeto poderia implementar para aumentar a colaboração da equipe?
				\item Como equilibrar autonomia técnica com decisões coletivas?
				\item Que tipo de estrutura de comunicação ajudaria a evitar retrabalho no projeto?
			\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
...
% SUBSECTION 3.6 ---------------------------------------------------------------
%-------------------------------------------------------------------------------
\subsection{Problema 6 - Modernização do Sistema de Matrícula de uma Universidade Federal}

\begin{frame}{Problema 6 - Modernização do Sistema de Matrícula de uma Universidade Federal}
	\begin{itemize}
		\justifying
		\item Uma universidade federal com 18 mil alunos enfrenta instabilidade no sistema de matrícula há anos. A cada semestre:
			...
			\end{itemize}
		\item Foi aprovado um projeto de R\$ 2 milhões para modernização da infraestrutura e desenvolvimento de novo sistema. O contrato prevê SLA (Acordo de Nível de Serviço) mínimo, mas não especifica detalhes técnicos de redundância. Durante a execução:
			\begin{itemize}
				\justifying
...
\begin{frame}{Problema 6 - Modernização do Sistema de Matrícula de uma Universidade Federal}
	\begin{itemize}
		\justifying
		\item \textbf{Dilema}: tecnicamente, a solução alternativa ainda atende o contrato, porém, pode comprometer estabilidade em períodos críticos.
		\item \textbf{Perguntas norteadoras}:
		\begin{itemize}
			\item \textbf{Compreensão}
				\begin{itemize}
					\justifying
					\item Qual é o objetivo principal do projeto de modernização do sistema de matrícula?
					\item Quais grupos de pessoas são impactados diretamente pela decisão sobre a infraestrutura do sistema?
					\item Qual é a diferença entre cumprir o contrato e agir com responsabilidade administrativa?
				\end{itemize}
			\item \textbf{Análise}
				\begin{itemize}
					\justifying
					\item Quais riscos técnicos e institucionais podem surgir se a empresa optar pela solução mais barata?
					\item De que forma uma falha no sistema pode impactar estudantes e a reputação da universidade?
					\item O fato de a solução alternativa não violar o contrato elimina a responsabilidade ética do gerente do projeto?
				\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Problema 6 - Modernização do Sistema de Matrícula de uma Universidade Federal}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\justifying
				\item Se você fosse o gerente do projeto, qual decisão tomaria em relação à infraestrutura do sistema?
				\item Como justificar sua decisão para a diretoria da empresa e para o cliente?
				\item Que mecanismos poderiam ser adotados para garantir transparência e responsabilidade em decisões semelhantes?
			\end{itemize}
		\end{itemize}
	\end{itemize}
\end{frame}
